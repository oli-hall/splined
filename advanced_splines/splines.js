...
Point.prototype.addNoise = function(noise) {
    this.x = move(this.x, 0, width, Math.floor(noise / 2));
...
function generatePoints(x, y) {
    var interval = Math.floor(x / points.length);
    for (i = 0; i < points.length; i++) {
        points[i] = new Point(interval * i, Math.floor(y / 2))
    }
    return points;
}
...
function updatePoints(points) {
    for (i = 1; i < points.length; i++) {
        points[i].addNoise(Math.min((i / 5) * noise, maxNoise));
    }
}
